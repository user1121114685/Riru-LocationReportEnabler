...
if [ ! -d "$CONFIG_PATH/properties" ]; then
    ...
    # 将 op_voice_recording_supported_by_mcc的返回值弄成1 启用通话录音
    echo -n "1" > "$CONFIG_PATH/properties/op_voice_recording_supported_by_mcc"
fi

if [ ! -d "$CONFIG_PATH/packages" ]; then
    ui_print "- Creating default configuration (2)"
    mkdir -p "$CONFIG_PATH/packages"
    touch "$CONFIG_PATH/packages/com.android.phone"
fi
